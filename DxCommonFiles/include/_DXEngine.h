...
typedef	struct { DWORD StackLevel, StackMax; SurfaceItemType *Stack; } SurfaceStackType ;
...
public:
	...
	IDirect3DDevice7*	GetD3DD(void)	{ return m_pD3DD; };
...
#ifdef	DEBUG_LOD_ID
	char	LodLabel[32];
	char			*GetLodUsedLabel(void) { return LodLabel; }
#endif

#ifdef	DEBUG_ENGINE	
	void DrawFrameSurfaces(float Alpha=1.0f);
	bool UseZBias;
	...
	void SetProjection(D3DXMATRIX *Settings) {	Projection=*Settings; m_pD3DD->SetTransform( D3DTRANSFORMSTATE_PROJECTION, (LPD3DMATRIX)&Projection ); }
	void SetWorld(D3DXMATRIX *Settings) {	World=*Settings;	}
	void SetViewport( DWORD l, DWORD t, DWORD r, DWORD b);
	void SetNVG(bool Status) { NVGStatus=Status; TheNVG.dvDirection=TheSun.dvDirection; }
	void SetTV(bool Status) { TVStatus=Status; TheTV.dvDirection=TheSun.dvDirection; }
	bool GetNVG(void) { return NVGStatus; }
	bool GetTV(void) { return TVStatus; }
	void SetFogLevel(float FogLevel) { m_FogLevel=FogLevel; }
	void SetFogColor(D3DCOLORVALUE *Color) { m_FogColor=*Color; }
	void CreateZeroTexture(void);
	void SelectTexture(GLint texID);
	void ClearLights(void) { TheLightEngine.ResetLightsList(); }
	DWORD	SetStencilMode(DWORD Stencil);
	void ClearStencil(void) { m_pD3DD->Clear(NULL,NULL,D3DCLEAR_STENCIL,0,1.0f,0); m_StencilRef=0; SetStencilMode(STENCIL_OFF); }
	...
	void SetSunColour(float r, float g, float b) { TheSunColour.r=r; TheSunColour.g=g; TheSunColour.b=b; }
	void SetSunLight(float Ambient, float Diffuse, float Specular);
	void SetSunVector(D3DVECTOR Direction) { TheSun.dvDirection=Direction; }
	void SetLODBias(float Bias) { m_LODBiasCx=Bias; }
	bool	NVGStatus,TVStatus;
	void	SetPitMode(bool Mode) { m_PitMode=Mode; }
	bool	GetPitMode(void) { return m_PitMode; }
...
private:
	...
	static	ObjectInstance		*m_TheObjectInstance;
	...
	void				SetScripts(bool state) { m_ScriptsOn=state; }
	...
	void	PushSurface(SurfaceStackType *Stack, D3DXMATRIX *State);
...
public:
	void	Draw3DPoint(D3DVECTOR *WorldPos, DWORD Color, bool Emissive=NULL);
	void	Draw3DLine(D3DVECTOR *WorldStart, D3DVECTOR *WorldEnd, DWORD ColorStart, DWORD ColorEnd, bool Emissive);
	...
	float				FogLevel(void) { return m_FogLevel; }
	float				LODBiasCx(void) { return m_LODBiasCx; }
...
public:
	bool	DX2D_GetVisibility(D3DXVECTOR3 *Pos, float Radius);
	float	DX2D_GetDistance(D3DXVECTOR3 *Pos, float Radius);
	void	DX2D_NextLayer( DWORD Flags = 0)	{ LayerSelected++; Layers[LayerSelected].Flags=Flags;  };
	void	DX2D_SetLayer( DWORD Layer=LAYER_NODRAW, DWORD Flags = 0)	{ if(Layer==LAYER_NODRAW) return; LayerSelected=Layer; Layers[LayerSelected].Flags=Flags;  };
