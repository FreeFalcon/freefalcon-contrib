...
public:
	...
	void			AssertValidModel(DWORD ID) { pVBuffers[ID].Valid=true; }
	...
	(BYTE*)			GetModelRoot(DWORD ID) { return ((BYTE*)pVBuffers[ID].Root); }
	IDirect3D7*		GetVBD3D(void)	{ return m_pD3D; };

	VBufferListType	*GetVbList(void) { return pVbList; };
