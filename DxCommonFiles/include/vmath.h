...
class Matrix
{
	...
	void SetViewMatrix(float fPitch, float fRoll, float fYaw, Vector vPos)
	{
		...
		mT.InitIdentity();
		mT.m[3][0]=-vPos.x; mT.m[3][1]=-vPos.y; mT.m[3][2]=-vPos.z;

		mX.InitIdentity();
		mX.m[0][0]=1; mX.m[0][1]=0; mX.m[0][2]=0;
		mX.m[1][0]=0; mX.m[1][1]=(float)cos(fPitch); mX.m[1][2]=(float)sin(fPitch);
		mX.m[2][0]=0; mX.m[2][1]=-(float)sin(fPitch); mX.m[2][2]=(float)cos(fPitch);

		mY.InitIdentity();
		mY.m[0][0]=(float)cos(fYaw); mY.m[0][1]=0; mY.m[0][2]=-(float)sin(fYaw);
		mY.m[1][0]=0; mY.m[1][1]=1; mY.m[1][2]=0;
		mY.m[2][0]=(float)sin(fYaw); mY.m[2][1]=0; mY.m[2][2]=(float)cos(fYaw);

		mZ.InitIdentity();
		mZ.m[0][0]=(float)cos(fRoll); mZ.m[0][1]=(float)sin(fRoll); mZ.m[0][2]=0;
		mZ.m[1][0]=-(float)sin(fRoll); mZ.m[1][1]=(float)cos(fRoll); mZ.m[1][2]=0;
		mZ.m[2][0]=0; mZ.m[2][1]=0; mZ.m[2][2]=1;
