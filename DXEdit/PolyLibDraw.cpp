...
/*	Pcolor		*rgba;
	...
	int	colorSet = TRUE;
#endif
	if(opFlag & PRIM_COLOP_COLOR) CV->AddUsedColor(rgbaIdx);
	if(opFlag & PRIM_COLOP_INTENSITY) CV->SetLite();
...
//	TheTextureBank.Select(TheStateStack.CurrentTextureTable[poly->texIndex]);
	...
	CV->AddPoly(PRIM_COLOP_TEXTURE,poly,poly->xyz,NULL,NULL,poly->uv,false);
	TheStateStack.context->SetPalID(0);
}
...
//	TheTextureBank.Select(TheStateStack.CurrentTextureTable[poly->texIndex]);
	CV->AddUsedTexture(poly->texIndex);								// Updates Texture Index
	CV->AddPoly(PRIM_COLOP_COLOR | PRIM_COLOP_INTENSITY | PRIM_COLOP_TEXTURE,poly,poly->xyz,poly->rgba,poly->I,poly->uv,false);
	TheStateStack.context->SetPalID(0);
}
