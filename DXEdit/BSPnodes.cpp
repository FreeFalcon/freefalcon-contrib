...
{
	// Fixup our dependents
	if((int)front!=-1) front	= RestorePointers( baseAddress, (int)front, tagListPtr );
	if((int)back!=-1) back = RestorePointers( baseAddress, (int)back,  tagListPtr );
}
	...
	*/							   
	if (ScriptNumber > 0) {
		...
		if (ScriptNumber < ScriptArrayLength) {
			...
			CV->SetScriptNumber(ScriptNumber);
		}
	}

	NodeCount=0;									// Node Counter;
	CV->SetupTextures(nTexIDs, pTexIDs, 8);
	GlobalBias=0;
	SubTreeCount=false;
	BSubTree::Draw();
	GlobalBias=0;
...
/*	ShiAssert( slotNumber < TheStateStack.CurrentInstance->ParentObject->nSlots );
	...
	CV->AddSlot(slotNumber, &rotation, &origin);
}
...
//	TheStateStack.Rotation=rotation;
	Pmatrix tempM = TheStateStack.Rotation;
	MatrixMult_Generic(&tempM,&rotation,&TheStateStack.Rotation);
	//TheStateStack.CompoundTransform( &R, &T );
	CV->OpenDof(ROTATE, dofNumber, rotation, translation);
	SubTreeCount=false;
	BSubTree::Draw();
	CV->CloseDof();
...
*/	TheStateStack.PushAll();
	...
	MatrixMult_Generic(&tempM,&rotation,&TheStateStack.Rotation);
//	TheStateStack.CompoundTransform( &R, &T );
	CV->OpenDof(XROTATE, dofNumber, min, max, multiplier, future, flags, rotation, translation);
	SubTreeCount=false;
	BSubTree::Draw();
	CV->CloseDof();
	TheStateStack.PopAll();
...
//	TheStateStack.CompoundTransform( &dofRot, &T );
	NodeCount++;									// Node Counter;
	CV->OpenDof(TRANSLATE, dofNumber, min, max, multiplier, future, flags, translation);
	SubTreeCount=false;
	BSubTree::Draw();
	CV->CloseDof();
	TheStateStack.PopAll();
...
//	TheStateStack.CompoundTransform( &dofRot, &T );
	NodeCount++;									// Node Counter;
	CV->OpenDof(SCALE, dofNumber, min, max, multiplier, future, flags, scale, translation);
	SubTreeCount=false;
	BSubTree::Draw();
	CV->CloseDof();
	TheStateStack.PopAll();
...
/*	ShiAssert( switchNumber < TheStateStack.CurrentInstance->ParentObject->nSwitches );
	...
	NodeCount++;									// Node Counter;
	mask=0xffffffff;													// Check for all childrens
	while (i < numChildren) {
		if( subTrees[i] ) // MLR 2003-10-12 changed from ShiAssert.
		{
			...
			if (mask & 1) {
				TheStateStack.PushVerts();
				CV->OpenDof(SWITCH, switchNumber, i);
	//			SubTreeCount=false;
				subTrees[i]->Draw();
				CV->CloseDof();
				TheStateStack.PopVerts();
...
/*	ShiAssert( switchNumber < TheStateStack.CurrentInstance->ParentObject->nSwitches );
	...
	while (i < numChildren) 
	{
		if(subTrees[i] )
		{
			...
			if (mask & 1) {
				TheStateStack.PushVerts();
				CV->OpenDof((flags & XSWT_REVERSED_EFFECT)?XSWITCH:SWITCH, switchNumber, i);
	//SubTreeCount=false;
				subTrees[i]->Draw();
				CV->CloseDof();
				TheStateStack.PopVerts();
