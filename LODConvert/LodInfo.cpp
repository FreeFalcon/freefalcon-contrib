...
CLodInfo::CLodInfo(CWnd* pParent, CString Title, Converter *conversion, BRoot* rt, BNodeType *TL, DWORD TC, DWORD Class, DWORD CTNumber, DWORD MType, DWORD VBClass)
	: CDialog(CLodInfo::IDD, pParent)
{
	...
	Report();
	ecd->Initialize(cv);
	ecd->Encode(ModelID, VBClass, CTNumber, MType);
	ECD=ecd;
	TheModelName=Title;
	...
	// *** WARN :THIS IS DONE TO REMEMBER TO ASSIGN VB CLASS WHEN SAVING **** DO NOT REMOVE !!
	// Header->VBClass=0; // OVERLOADED BY FUNCTION
	// ***************************************************************************************
