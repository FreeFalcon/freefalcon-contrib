...
{
    ShiAssert(w>= 0 && w < NumWeaponTypes);
    // if (F4IsBadReadPtr(&(WeaponDataTable[w]), sizeof(WeaponClassDataType)) || F4IsBadReadPtr(&(WeaponDataTable[w].HitChance[mt]), sizeof(uchar))) // JB 011205 (too much CPU) // JB 010331 CTD
    if (w < 0 || w >= NumWeaponTypes) // JB 011205
	return 0;
...
int GetListEntryWeapon(int list, int num)
{
    ...
    if(list< 0 || list >= NumWeaponTypes) return 0;
