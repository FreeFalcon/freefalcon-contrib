/*
 * Machine Generated source file for message "Request Aircraft Slot".
...
UI_RequestAircraftSlot::UI_RequestAircraftSlot(VU_MSG_TYPE type, VU_ID senderid, VU_ID target) : FalconEvent (RequestAircraftSlot, FalconEvent::SimThread, senderid, target)
{
	RequestOutOfBandTransmit();
	RequestReliableTransmit ();
   // Your Code Goes Here
...
UI_RequestAircraftSlot::~UI_RequestAircraftSlot(void)
{
   // Your Code Goes Here
}
...
int UI_RequestAircraftSlot::Process(uchar autodisp)
	...
	//ShiAssert(requester->GetFlyState() == FLYSTATE_IN_UI);
#endif
...
int UI_RequestAircraftSlot::EmptyFlightSlot (Flight flight)
	...
	if (dataBlock.requested_slot < 255)
		...
		flight->MakeFlightDirty (DIRTY_PILOTS, DDP[153].priority);
	...
	if (FalconLocalGame->GetGameType() == game_Dogfight)
		...
		flight->MakeFlightDirty (DIRTY_PILOTS, DDP[154].priority);
		//	flight->MakeFlightDirty (DIRTY_PILOTS, SEND_RELIABLEANDOOB);
			...
			for (i=0; i<PILOTS_PER_FLIGHT && !retval; i++)
			{
				if (flight->plane_stats[i] == AIRCRAFT_NOT_ASSIGNED)
				{
					...
					if (dataBlock.request_type == REQUEST_SLOT_JOIN_PLAYER)
					{
						...
						flight->MakeFlightDirty (DIRTY_PILOTS, DDP[156].priority);
						//	flight->MakeFlightDirty (DIRTY_PILOTS, SEND_RELIABLEANDOOB);
						flight->MakeFlightDirty (DIRTY_PLANE_STATS, DDP[157].priority);
					...
					else
					{
						...
						flight->MakeFlightDirty (DIRTY_PILOTS, DDP[158].priority);
						//	flight->MakeFlightDirty (DIRTY_PILOTS, SEND_RELIABLEANDOOB);
					}
