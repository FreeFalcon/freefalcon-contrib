/*
 * Machine Generated source file for message "AWACS Message".
...
// 2002-03-07 MN search for closest carrier instead of the home carrier
			if (flight)
			{
				...
				while (nu)
				{
					cf = nu;
					nu = (Unit) myit.GetNext();
					if (cf->GetTeam() != plane->GetTeam() || cf->Dead())
						continue;
					if (!cf->IsTaskForce())
						continue;
...
//			SendCallToAWACS(plane, rcDECLARE);
         otherThing = (FalconEntity *)vuDatabase->Find(dataBlock.caller); // 2002-02-28 MODIFIED BY S.G. Replaced CampBaseClass to FalconEntity since we don't know if it's a sim or campaign object yet
         ...
         if (!otherThing)
         {
			...
			if (theRadar)
			{
			   ...
			   if (targetId == FalconNullId && theRadar->CurrentTarget())
			   {
...
// flight and got identified (only flights can be "GetIdentified = true", so this checks also if it is a flight/aircraft)
			{
	               ...
	               else if (randNum < 5)
				   {
		              radioMessage->dataBlock.edata[4] = 2;
  					  radioMessage->dataBlock.message = rcGENERALID;
					       ...
					       GetTTRelations ( otherThing->GetTeam(), plane->GetTeam() ) < War)
				...
				if (simThing)
				{
					...
					radioMessage->dataBlock.edata[5] = (short)((simThing->Type() - VU_LAST_ENTITY_TYPE)*2);	//type
				}
				else 
				{
					...
					if(numAircraft > 1)
						radioMessage->dataBlock.edata[5] = (short)(((Unit)campThing)->GetVehicleID(0)*2 + 1);	//type
					else
						radioMessage->dataBlock.edata[5] = (short)(((Unit)campThing)->GetVehicleID(0)*2);	//type
				}
			}
			else 
// unidentified flights and others
			{
// 2002-02-11 MN changed to CreateCallFromAwacsPlane - AWACS issues the exact aircraft ID in the flight (not just the flight number)
...
/*
						...
						ObjectiveClass* curObj = (ObjectiveClass*) findWalker.GetFirst();
						D3DFrame::Vector vPosFlight(flight->XPos(), flight->YPos(), flight->ZPos());
						float fMinDistance = (float) ULONG_MAX;
						ObjectiveClass *bestObj = NULL;
						...
						while(curObj)
						{
							...
							{
								// curObj->brain->Exec();
								D3DFrame::Vector vPos(curObj->XPos(), curObj->YPos(), curObj->ZPos());
								float fDist = (vPos - vPosFlight).Size();
						...
						if(bestObj)
						{
							...
							if(rx > 0.0F && fabs(ry) > testRad && range < 0.0F && testObject->GetCampaignObject() == self->GetCampaignObject() && self->vehicleInUnit > ((AircraftClass*)testObject)->vehicleInUnit)
							{
								xft = trackX - af->x;
								yft = trackY - af->y;
								// get relative position and az
