...
FalconAirTaskingMessage::FalconAirTaskingMessage(VU_ID entityId, VuTargetEntity *target, VU_BOOL loopback) : FalconEvent (AirTaskingMsg, FalconEvent::CampaignThread, entityId, target, loopback)
	{
	// Your Code Goes Here
	}

FalconAirTaskingMessage::FalconAirTaskingMessage(VU_MSG_TYPE type, VU_ID senderid, VU_ID target) : FalconEvent (AirTaskingMsg, FalconEvent::CampaignThread, senderid, target)
	{
	// Your Code Goes Here
	type;
	}
...
int FalconAirTaskingMessage::Process(uchar autodisp)
	{
	if (autodisp || !TheCampaign.IsLoaded())		// I shouldn't really be getting messages if we're not loaded!?!
		return -1;
