...
FalconATCCmdMessage::FalconATCCmdMessage(VU_ID entityId, VuTargetEntity *target, VU_BOOL loopback) : FalconEvent (ATCCmdMsg, FalconEvent::SimThread, entityId, target, loopback)
{
	// Your Code Goes Here
//me123	RequestOutOfBandTransmit ();
}

FalconATCCmdMessage::FalconATCCmdMessage(VU_MSG_TYPE type, VU_ID senderid, VU_ID target) : FalconEvent (ATCCmdMsg, FalconEvent::SimThread, senderid, target)
{
	// Your Code Goes Here
	type;
}
...
int FalconATCCmdMessage::Process(uchar autodisp)
{
	...
	float finalX=0.0F, finalY=0.0F, baseX=0.0F, baseY=0.0F, x=0.0F ,y=0.0F, z=0.0F, dx=0.0F, dy=0.0F, distD=0.0F, distA=0.0F;
	GridIndex	X=0, Y=0;
			...
			case Divert:
				...
				if(divertBase)
				{
					if(divertBase->GetType () == TYPE_AIRBASE)
					{
						...
						radioMessage->dataBlock.edata[4] = az/*SimToGrid(divertBase->YPos())*/;
						radioMessage->dataBlock.edata[5] = rangediv/*SimToGrid(divertBase->XPos())*/;
