...
int FalconDivertMessage::Process(uchar autodisp)
	...
	if (dataBlock.mission > 0)
		...
		if ((!target || (target->IsUnit() && ((UnitClass*)target)->Dead())) && dataBlock.mission > 0)
			return 0;
...
int CheckDivertStatus (int reply)
	...
	if (!sNextRepost)
		...
		if (!target || (target->IsUnit() && (((Unit)target)->Dead() || ((Unit)target)->Broken())))
			return 0;
...
//		}
	...
	if (SimDriver.playerEntity && flight->GetComponentLead() != SimDriver.playerEntity)
		{
