/*
 * Machine Generated source file for message "Send Objective Data".
...
FalconSendObjData::~FalconSendObjData(void)
	{
	delete[] dataBlock.objData;
   	}

int FalconSendObjData::Size (void) 
	{ 
...
int FalconSendObjData::Decode (VU_BYTE **buf, int length)
	{
	int size = 0;
	...
	memcpy (dataBlock.objData, *buf, dataBlock.size);	*buf += dataBlock.size;			size += dataBlock.size;
...
int FalconSendObjData::Encode (VU_BYTE **buf)
	...
	memcpy (*buf, dataBlock.objData, dataBlock.size);	*buf += dataBlock.size;			size += dataBlock.size;
...
int FalconSendObjData::Process(uchar autodisp)
{
	...
	if (TheCampaign.Flags & CAMP_NEED_OBJ_DELTAS)
	{
		...
		DecodeObjectiveDeltas((VU_BYTE**) &bufptr, session);
