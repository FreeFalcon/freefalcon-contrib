/*
 * Machine Generated source file for message "Send Eval Message".
...
SendEvalMessage::~SendEvalMessage(void)
	{
	if (dataBlock.data)
		delete dataBlock.data;
	}

int SendEvalMessage::Size (void)
	{
    ShiAssert(dataBlock.size >= 0);
	return	(FalconEvent::Size() +	sizeof (int) + sizeof(ushort) + dataBlock.size);
	}

int SendEvalMessage::Decode (VU_BYTE **buf, int length)
	{
	int		size;

	size = FalconEvent::Decode (buf, length);
	memcpy (&dataBlock.message, *buf, sizeof(int));				*buf += sizeof(int);			size += sizeof(int);		
