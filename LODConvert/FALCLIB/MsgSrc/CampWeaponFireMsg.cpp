...
int FalconCampWeaponsFire::Size (void) 
	{ 
    	ShiAssert ( dataBlock.size >= 0 );
	return	FalconEvent::Size() +
			sizeof(VU_ID) +
			sizeof(VU_ID) +
			sizeof(dataBlock.weapon) +
			sizeof(dataBlock.shots) + 
			sizeof(uchar) +
			sizeof(uchar) +
			sizeof(ushort) +
			dataBlock.size;
...
int FalconCampWeaponsFire::Decode (VU_BYTE **buf, int length)
	{
	int size = 0;

	memcpy (&dataBlock.shooterID, *buf, sizeof(VU_ID));					*buf += sizeof(VU_ID);						size += sizeof(VU_ID);
	memcpy (&dataBlock.fWeaponUID, *buf, sizeof(VU_ID));				*buf += sizeof(VU_ID);						size += sizeof(VU_ID);
	memcpy (dataBlock.weapon, *buf, sizeof dataBlock.weapon);	*buf += sizeof dataBlock.weapon;	size += sizeof dataBlock.weapon;
	memcpy (dataBlock.shots, *buf, sizeof dataBlock.shots);	*buf += sizeof dataBlock.shots;	size += sizeof dataBlock.shots;
	memcpy (&dataBlock.fPilotId, *buf, sizeof(uchar));					*buf += sizeof(uchar);						size += sizeof(uchar);		
	memcpy (&dataBlock.dPilotId, *buf, sizeof(uchar));					*buf += sizeof(uchar);						size += sizeof(uchar);		
...
int FalconCampWeaponsFire::Encode (VU_BYTE **buf)
	...
	int size = 0;
    	...
    	ShiAssert ( dataBlock.size >= 0 );
	memcpy (*buf, &dataBlock.shooterID, sizeof(VU_ID));					*buf += sizeof(VU_ID);						size += sizeof(VU_ID);
	memcpy (*buf, &dataBlock.fWeaponUID, sizeof(VU_ID));				*buf += sizeof(VU_ID);						size += sizeof(VU_ID);
	memcpy (*buf, dataBlock.weapon, sizeof dataBlock.weapon);	*buf += sizeof dataBlock.weapon;	size += sizeof dataBlock.weapon;
	memcpy (*buf, dataBlock.shots, sizeof dataBlock.shots);	*buf += sizeof dataBlock.shots;	size += sizeof dataBlock.shots;
	memcpy (*buf, &dataBlock.fPilotId, sizeof(uchar));					*buf += sizeof(uchar);						size += sizeof(uchar);		
	memcpy (*buf, &dataBlock.dPilotId, sizeof(uchar));					*buf += sizeof(uchar);						size += sizeof(uchar);		
...
void DoDistanceVisualEffects (CampEntity shooter, CampEntity target, int weapon_id, int shots)
	...
	player = FalconLocalSession->Camera(0);
...
FireMissileAtSim( CampEntity shooter, SimBaseClass *simTarg, short weapId)
{
...
#endif
	tmpTargetPtr->Reference( SIM_OBJ_REF_ARGS );
	...
	tmpTargetPtr->Release( SIM_OBJ_REF_ARGS );

	// put the missile into the world
	vuDatabase->Insert(theMissile);
