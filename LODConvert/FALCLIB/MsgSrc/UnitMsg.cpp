/*
 * Machine Generated source file for message "Unit Messages".
...
FalconUnitMessage::FalconUnitMessage(VU_ID entityId, VuTargetEntity *target, VU_BOOL loopback) : FalconEvent (UnitMsg, FalconEvent::CampaignThread, entityId, target, loopback)
	{
	// Your Code Goes Here
	}
	...
	switch (dataBlock.message)
		...
		case unitSetVehicles:
			if (dataBlock.data1 < VEHICLES_PER_UNIT)
				...
				for (i=VEHICLES_PER_UNIT-1; i>=0 && dataBlock.data2; i--)
					{
			...
			if (u->GetType() == TYPE_FLIGHT && u->GetDomain() == DOMAIN_AIR)
				...
				for (i=0; i<PILOTS_PER_FLIGHT; i++)
					{
					if (((Flight)u)->slots[i] > VEHICLES_PER_UNIT || ((Flight)u)->slots[i] == dataBlock.data1)
						{
