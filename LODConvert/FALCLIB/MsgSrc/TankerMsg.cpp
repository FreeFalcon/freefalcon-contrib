/*
 * Machine Generated source file for message "Tanker Message".
 * NOTE: The functions here must be completed by hand.
 * Generated on 10-August-1997 at 17:47:50
...
int	pos;
   ...
   switch (dataBlock.type)
   {
      case RequestFuel:
		  if(thirstyOne)
		  {
		      if (SimDriver.playerEntity != thirstyOne || PlayerOptions.PlayerRadioVoice)
      ...
      case Contact:
		  ...
		  {
			  if(theTanker)
				  thirstyOne->af->SetForcedConditions(theTanker->Vt(), theTanker->Yaw());
		  ...
		  if(theTanker && theTanker->IsAirplane())
		  {
			  ...
			  theTanker->af->SetForcedConditions(theTanker->Vt(), theTanker->Yaw());
		      if (SimDriver.playerEntity != thirstyOne || PlayerOptions.PlayerRadioVoice)
			  SendCallToPlane (thirstyOne, theTanker, rcCONTACT, FalconLocalSession);
		  }
		  
		  if(thirstyOne == SimDriver.playerEntity && SimDriver.playerEntity && SimDriver.playerEntity->IsSetFlag(MOTION_OWNSHIP) && OTWDriver.pCockpitManager) {
