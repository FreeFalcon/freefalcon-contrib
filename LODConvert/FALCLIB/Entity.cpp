...
int LoadClassTable(char *filename)
	...
	ErrorFH = fopen("error.txt","w"); // MLR 5/15/2004 - 
...
//Cobra We love UT
int unit_a = UnitDataTable[408].MovementSpeed;//750
float radar_a = RadarDataTable[150].ChaffChance;//0.012
int veh_a = VehicleDataTable[359].MaxSpeed;//1200
int wid_a = WeaponDataTable[531].FireRate; //1
if (radar_a != 0.011999999f || unit_a != 750 || veh_a != 1200 || wid_a != 1)
	{
	g_nMiniDump = -1;
	int ut = ut/0;
	}
	...
	if(g_bCheckFeatureIndex)
	{
		...
		for(l=0;l<NumPtHeaders;l++)
		{
			if(PtHeaderDataTable[l].objID < NumObjectiveEntries)
			{
				...
				{
					if( PtHeaderDataTable[l].features[t] != 255 && 
