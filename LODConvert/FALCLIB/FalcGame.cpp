...
FalconGameEntity::FalconGameEntity(VU_BYTE** stream) : VuGameEntity (0,"VuGame")
	{
	VU_ID			sessionid(0, 0);
...
//	    AddSession(sessionid);
	...
	memcpy (&gameType, *stream, sizeof (FalconGameType));	*stream += sizeof (FalconGameType);
	memcpy (&rules,	*stream, sizeof (class RulesClass));	*stream += sizeof (class RulesClass);
	memcpy (&size, *stream, sizeof (uchar));				*stream += sizeof (uchar);
	gameName_ = new char[size+1];
	memcpy (gameName_, *stream, sizeof(char)*size);		*stream += sizeof (char)*size;
	gameName_[size]=0;
	}
...
int FalconGameEntity::LocalSize (void)
	{
...
VU_ERRCODE FalconGameEntity::Handle(VuFullUpdateEvent *event)
	{
	FalconGameEntity	*tmpGame = (FalconGameEntity*)(event->expandedData_);
