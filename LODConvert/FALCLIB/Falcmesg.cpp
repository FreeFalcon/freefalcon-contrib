...
*/
	...
	if(theEvent->Type() == RadioChatterMsg)
	{
		...
		FalconEntity *from = (FalconEntity*)vuDatabase->Find(((FalconRadioChatterMessage*)theEvent)->dataBlock.from);

//		MonoPrint("\nSending Chatter Message ID: %d\n", radioMessage->dataBlock.message );
...
if (theEvent->Target() == (VuTargetEntity*) vuLocalSessionEntity) printit = 0;
...
#define MonoprintMessageStats 1
#ifdef MonoprintMessageStats
...
//MonoPrint("TOTreliable %d, TOToob %d, TOTtotal %d \n", TOTsendreliable, TOToob, TOTsendtotal);
 if ((printit) && count > 5 && theEvent->Type() != 30) MonoPrint("Avgreliable %d, Avgoob %d, Avgtotal %d mescount %d \n", (TOTsendreliable/count), (TOToob/count), (TOTsendtotal/count),msgcount);
	sendreliable = 0;
	msgoob = 0;
	sendtotal = 0;
...
int FalconEvent::Size (void)
	{
	return FalconEvent::LocalSize() + VuMessage::Size();
	}

int FalconEvent::LocalSize (void)
	{
	return sizeof(HandlingThread);
	}

int FalconEvent::Decode (VU_BYTE **buf, int length)
	{
	int size;
	long start = (long) *buf;

	size = VuMessage::Decode (buf, length);
	memcpy (&handlingThread, *buf, sizeof (HandlingThread));
...
FalconMessageFilter::FalconMessageFilter(FalconEvent::HandlingThread theThread, ulong vuMessageBits)
	...
	vuFilterBits = vuMessageBits | VU_DELETE_EVENT_BITS;	// KCK: All threads must handle delete events
	}
...
VU_BOOL FalconMessageFilter::Test(VuMessage *event)
	{
	uchar retval = TRUE;
...
VuMessageFilter* FalconMessageFilter::Copy()
	{
