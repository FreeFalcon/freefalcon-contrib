...
/*			sprintf(szFile,"%sKoreaObj.dxh", KoreaObjFolder); 
		...
		while(c < hdr.nLOD)
		{
			...
			FileType = LODUsedByCT((int) c, (int*) &Domain, (int*) &Class, (int*)&Type, (int*)&CTNumber, (int*)&CTIndex, (int*)&LodLevel, (int*)&ParentLevel, &LODName);
...
int	CDBConverter::LODUsedByCT(int eLOD, int* Domain, int* Class, int* Type, int* CTNumnber, int *CTIndex, int *LodLevel, int *ParentLevel, CString *Name)
{
			int					c, d, e;
			int					FileType = 0;
      int					grDat, nL;
