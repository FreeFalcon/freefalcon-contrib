...
class ObjectInstance {
  public:
	...
	void	SetSwitch( int id, UInt32 value )	{ ShiAssert(id<ParentObject->nSwitches); SwitchValues[id] = value; };
	void	SetDOFrotation( int id, float r )	{ ShiAssert(id<ParentObject->nDOFs); DOFValues[id].rotation = r; };
	void	SetDOFxlation( int id, float x )	{ ShiAssert(id<ParentObject->nDOFs); DOFValues[id].translation = x; };
	void	SetSlotChild( int id, ObjectInstance *o )	{ ShiAssert(id<ParentObject->nSlots); SlotChildren[id] = o; };
