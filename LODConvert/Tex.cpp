...
BOOL Texture::LoadImage(char *filename, DWORD newFlags, BOOL addDefaultPath)
{
	...
	if(texFile.imageType == IMAGE_TYPE_APL) flags |= MPR_TI_ALPHA;
...
bool Texture::CreateTexture(char *strName)
{
	...
	// JB 010318 CTD
	if( (!palette)&&/*!F4IsBadReadPtr(palette,sizeof(Palette)) && */(flags & MPR_TI_PALETTE) )
	{
		palette->Activate();
		ShiAssert(palette->palHandle);

		// JB 010616
		if(!palette->palHandle) return false;
...
bool TextureHandle::Create(char *strName, UInt32 info, UInt16 bits, UInt16 width, UInt16 height, DWORD dwFlags)
{
	...
	try
	{
		...
		// JB 010326 CTD
		if( 0/*F4IsBadReadPtr(m_pD3DHWDeviceDesc,sizeof(_D3DDeviceDesc7))*/ )
		{
		...
		if(dwFlags & FLAG_RENDERTARGET)
		{
			...
			if(1 || rc->m_eDeviceCategory >= DXContext::D3DDeviceCategory_Hardware)
			{
		...
		if( 1 || rc->m_eDeviceCategory >= DXContext::D3DDeviceCategory_Hardware )
		{
			if( !(dwFlags & FLAG_NOTMANAGED) )
			{
				...
				if(1)//if(g_bEnableNonPersistentTextures)
...
bool TextureHandle::Reload()
{
	...
	try
	{
		...
		// JB 010305 CTD
		if(0/*F4IsBadReadPtr(m_pDDS,sizeof(IDirectDrawSurface7))*/) return false;

		HRESULT hr = m_pDDS->Lock(NULL,&ddsd,DDLOCK_WAIT | DDLOCK_WRITEONLY | DDLOCK_SURFACEMEMORYPTR,NULL);
...
// Reloading is a different story - because this is called VERY frequently we have to be very fast with whatever we are doing here
	...
	if( m_dwFlags&MPR_TI_DDS )
	{
		...
		for(int i = 0; i < m_nImageDataStride; i++) *pDst++ = *pSrc++;
	}
	else
		switch(m_eSurfFmt)
		{
			...
			case D3DX_SF_A1R5G5B5:
			{
				...
				// JB 010404 CTD
				if(0/*F4IsBadReadPtr(pSrc,sizeof(BYTE)) || F4IsBadReadPtr(pDst,sizeof(WORD))*/) break;
